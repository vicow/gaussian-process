...
def learning_curve(seed=2, runs=10, light=False, K=2, outlier_threshold=10, granularity=1.0):
    ...
    for r in range(runs):
        ...
        rmse_run, accuracy_run = one_run(projects_train, projects_test, K, outlier_threshold, granularity)
        ...
        with open('rmse_increments_outlier_%s_granularity_%s.pkl' % (outlier_threshold, granularity), 'wb') as f:
            cp.dump(rmse_all, f)
        with open('accuracy_increments_outlier_%s_granularity_%s.pkl' % (outlier_threshold, granularity), 'wb') as f:
