...
class CrossValidationResults(object):
    ...
    def plot(self, filename):
        ...
        plt.fill_between(self.components, self.means - self.stderrs, self.means + self.stderrs, facecolor='blue',
                         alpha=0.2)
        plt.title('Avg. Out-of-Sample Performance For Regression Mixtures\n([{0}, {1}] components, {2} folds)'.format(
                self.min_components, self.max_components, self.num_folds))
...
def fit_mixture(data, keys, num_components, max_iterations, stochastic=False, verbose=False, threshold=0.00001):
    ...
    """
    num_features = list(data.values())[0][0].shape[1]
...
def fit_with_restarts(data, keys, num_components, max_iterations, num_restarts, stochastic=False, verbose=False,
                      num_workers=1):
    """
    ...
    if num_workers > 1:
        ...
        results = [fit_mixture(data, keys, num_components, max_iterations, stochastic=stochastic, verbose=verbose) for _
                   in range(num_restarts)]
...
def fit_with_restarts_worker(worker_params):
    ...
    return fit_with_restarts(data, keys, num_components, max_iterations, num_restarts, stochastic=stochastic,
                             verbose=verbose)


def cross_validate(data, keys, num_folds, min_components, max_components, max_iterations, stochastic=False,
                   verbose=False, initialization_trials=5, num_workers=1):
    """
    ...
    for fold, (training_data, testing_data) in enumerate(folds):
        ...
        if num_workers == 1:
            ...
            for num_components in results.components:
                ...
                component_results.append(fit_with_restarts(training_data, training_keys, num_components, max_iterations,
                                                           initialization_trials, stochastic=stochastic,
                                                           verbose=verbose))
        else:
            # Fit each component in parallel
            if verbose:
                print('\t\tFitting with {0} processes'.format(num_workers))
            worker_params = [(training_data, training_keys, num_components, max_iterations, initialization_trials,
                              stochastic, verbose) for num_components in results.components]
        ...
        for max_result in component_results:
            ...
            oos_log_likelihood = out_of_sample_log_likelihood(testing_data, testing_keys, model.component_weights,
                                                              model.coefficients, model.variances)
...
def save_results(results, keys, filename):
    ...
    with open(filename, 'wb') as f:
        ...
        print('component_weights: {0} coefficients: {1} variances: {2}'.format(best.component_weights.shape,
                                                                               best.coefficients.shape,
                                                                               best.variances.shape))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
            description='Fits a finite mixture of linear regression models over sets of sets of data points.')

    parser.add_argument('datafile', help='The file containing the data.')
    parser.add_argument('--num_components', type=int,
                        help='The number of linear regression mixture components. If not specified, cross-validation will be used to determine the number of components.')
    parser.add_argument('--outfile', default='results.csv',
                        help='The file to output the resulting model parameters and point assignments.')
    parser.add_argument('--stochastic', dest='stochastic', action='store_true',
                        help='Use stochastic sampling for point classification. Deterministic EM assigns points to their MAP component at every iteration; stochastic EM samples proportional to likelihood. After max_iterations is over, the MAP assignments will be calculated based on the parameters of the last iteration.')
    parser.add_argument('--max_iterations', type=int, default=1000,
                        help='The maximum number of EM iterations to perform.')
    parser.add_argument('--min_components', type=int, default=1,
                        help='The minimum number of mixture components to use during cross-validation.')
    parser.add_argument('--max_components', type=int, default=20,
                        help='The maximum number of mixture components to use during cross-validation.')
    parser.add_argument('--num_folds', type=int, default=10,
                        help='The number of folds to use when performing n-fold cross-validation.')
    parser.add_argument('--num_restarts', type=int, default=5, help='The number of random restarts to try.')
    parser.add_argument('--plot_cross_validation',
                        help='Plot the results of n-fold cross-validation and save the results to file.')
    parser.add_argument('--verbose', dest='verbose', action='store_true',
                        help='Print detailed progress information to the console.')
    parser.add_argument('--num_workers', type=int, default=1,
                        help='The number of parallel processes to use when performing cross-validation.')
    ...
    # Deterministic EM assigns points to their MAP component at every iteration.
    # Stochastic samples proportional to likelihood.
    ...
    if num_components is None:
        print('Performing {0}-fold cross-validation to find the best component count in the range [{1},{2}]'.format(
                args.num_folds, args.min_components, args.max_components))
        cv_results = cross_validate(data, keys, args.num_folds, args.min_components, args.max_components,
                                    args.max_iterations, stochastic=use_stochastic_em,
                                    initialization_trials=args.num_restarts, verbose=verbose,
    ...
    print('Fitting a mixture of {0} components with {1} restarts parallelized over {2} processes'.format(num_components,
                                                                                                         args.num_restarts,
                                                                                                         args.num_workers))
    results = fit_with_restarts(data, keys, num_components, args.max_iterations, args.num_restarts,
                                stochastic=use_stochastic_em, verbose=verbose, num_workers=args.num_workers)
